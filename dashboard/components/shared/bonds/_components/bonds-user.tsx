...
const BondsUsers = () => {
    ...
    const getData = useCallback(async () => {
        const url = (debouncedAddress && debouncedAddress !== "")
            ? `/api/bonds-user?address=${encodeURIComponent(debouncedAddress)}`
            : `/api/bonds-user`;
                ...
                <tbody className="text-slate-300">
                    {queryData.data?.map((v, k) =>
                        <tr key={k} className="border-t border-slate-700">
                            ...
                            <td className="p-3">{formatNumber(formatDecimal(v.amount, -6), false, 0)} BABY</td>
