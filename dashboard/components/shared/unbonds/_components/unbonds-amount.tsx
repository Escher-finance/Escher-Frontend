...
const UnbondsAmount = (props: Props) => {

    const getData = useCallback(async () => {
        const response = await fetch(`/api/unbonds-amount`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
        });
        const responseJson = (await response.json() as { unbond_date: string, total_unbond_amount: number }[]);

        const result: Data[] = [];
        responseJson.map(v => {
            let amount = v.total_unbond_amount;
            if (props.weight) {
            ...
            result.push({
                ...
                total_unbond_amount: amount ? formatDecimal(amount, -6) : undefined,
                total_unbond_amount_formatted: amount ? formatNumber(formatDecimal(amount, -6)) : undefined,
            });
        });
    ...
    const queryData = useQuery({
        queryKey: ["unbondsAmountHistory", props.weight],
        queryFn: getData,
    ...
    const data = useMemo((): Data[] | undefined => {
        ...
        const result: Data[] = Array.from({ length: 30 }, (_, i) => {
            ...
            return {
                unbond_date: date,
                total_unbond_amount: data?.total_unbond_amount,
                total_unbond_amount_formatted: data?.total_unbond_amount_formatted
            }
        }).reverse();
    ...
    return (
        <div className="w-full h-full mt-8" onClick={() => console.log({ queryData, data })}>
            {(data !== undefined) ?
