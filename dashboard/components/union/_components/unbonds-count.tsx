...
const UnionUnbondsCount = () => {
    const getData = useCallback(async () => {
        const response = await fetch(`/api/union/unbonds`, {
            method: 'GET',
    ...
    const data = useMemo((): Data[] | undefined => {
        ...
        const result: Data[] = Array.from({ length: 30 }, (_, i) => {
            ...
            return { unbond_date: date, total_unbond: data?.total_unbond };
        }).reverse();
        return result
    }, [queryData.data]);
    ...
    const maxData = useMemo(() => {
        ...
        return Math.max(...queryData.data.map(v => (v.total_unbond ?? 0))) + 5;
    }, [queryData.data]);
...
export default UnionUnbondsCount;
