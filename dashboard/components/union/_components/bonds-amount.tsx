...
const UnionBondsAmount = (props: Props) => {
    const getData = useCallback(async () => {
        const response = await fetch(`/api/union/bonds-amount`, { headers: { 'Content-Type': 'application/json' } });
        const responseJson = (await response.json() as { bond_date: string, total_bond_amount: string }[]);

        const result: Data[] = [];
        responseJson.map(v => {
            let amount = Number(v.total_bond_amount);
            if (props.weight) {
                amount = props.weight / 100 * amount
            }
            result.push({
                bond_date: formatDate(v.bond_date, "D MMM"),
                total_bond_amount: formatDecimal(amount, -18),
                total_bond_amount_formatted: formatNumber(formatDecimal(amount, -18)),
            });
        });
        return result;
    }, [props.weight]);
    ...
    const queryData = useQuery({
        queryKey: ["unionBondsAmountHistory", props.weight],
        queryFn: getData,
    ...
    const data = useMemo((): Data[] | undefined => {
        ...
        const result: Data[] = Array.from({ length: 30 }, (_, i) => {
            ...
            return { bond_date: date, total_bond_amount: data?.total_bond_amount, total_bond_amount_formatted: data?.total_bond_amount_formatted };
        }).reverse();
        return result
    }, [queryData.data]);
            ...
            {(data !== undefined) ?
                <ResponsiveContainer height={450}>
                    <BarChart data={data} margin={{ top: 0, right: 0, bottom: 0, left: 20 }}>
                        ...
                        <YAxis type="number" padding={{ top: 0, bottom: 0 }} width={30} tickFormatter={(value) => formatNumber(Number(value), true, 0)} />
                        <Tooltip content={<CustomTooltip />} cursor={false} />
                        <Bar dataKey="total_bond_amount" fill="#0ea5e9" label={({ x, y, value, width }) => (
...
export default UnionBondsAmount;
