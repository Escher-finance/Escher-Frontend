...
import { useUnionTraceBond } from "@/hooks/unionIndexer/bond";
...
const UnionTraceBond = (props: TraceProps) => {
    const lst = props.lst ?? "babylon";

    const totalLine = 26;
    const [showDetails, setShowDetails] = useState(true);

    const { data: unionTraceBond } = useUnionTraceBond(props.successTxHash);
    ...
    const curTrace = useMemo(() => {
        const hashes = [
            unionTraceBond?.bond_traces?.find(v => v.type === "PACKET_SEND")?.transaction_hash ?? undefined,
            unionTraceBond?.bond_traces?.find(v => v.type === "PACKET_RECV")?.transaction_hash ?? undefined,
            unionTraceBond?.bond_traces?.find(v => v.type === "PACKET_ACK")?.transaction_hash ?? undefined,

            unionTraceBond?.delivery_traces?.find(v => v.type === "PACKET_SEND")?.transaction_hash ?? undefined,
            unionTraceBond?.delivery_traces?.find(v => v.type === "PACKET_RECV")?.transaction_hash ?? undefined,
            unionTraceBond?.delivery_traces?.find(v => v.type === "PACKET_ACK")?.transaction_hash ?? undefined,
        ];

        return {
            percentage: ((hashes.filter(v => v !== undefined).length + (props.source === "local" ? 2 : 0)) / totalPhase * 100)
        };
    }, [props.source, totalPhase, unionTraceBond?.bond_traces, unionTraceBond?.delivery_traces]);

    const getFinishHash = useCallback(() => {
        let hash: string | undefined;
        if (unionTraceBond?.delivery_success) {
            hash = unionTraceBond?.delivery_traces?.find(v => v.type === "PACKET_ACK")?.transaction_hash ??
                unionTraceBond?.delivery_traces?.find(v => v.type === "PACKET_RECV")?.transaction_hash ??
                unionTraceBond?.delivery_traces?.find(v => v.type === "PACKET_SEND")?.transaction_hash;
        }

        return hash;
    }, [unionTraceBond?.delivery_success, unionTraceBond?.delivery_traces])
                        ...
                        <AccordionContent className="p-0">
                            ...
                            <div className={clsx(
                                ...
                                <TraceComp
                                    border="left"
                                    type="PACKET_SEND"
                                    traces={unionTraceBond?.bond_traces}
                                    title={"Send packet"}
                                    isTransactionPage={props.isTransactionPage}
                                />
                                <TraceComp
                                ...
                                <TraceComp
                                    border="right"
                                    type="PACKET_SEND"
                                    traces={unionTraceBond?.delivery_traces}
                                    title={"Send packet"}
                                    isTransactionPage={props.isTransactionPage}
                                />
                                <TraceComp
                            ...
                            {unionTraceBond?.packet_hash &&
                                <Link
                                    href={`https://app.union.build/explorer/packets/${unionTraceBond?.packet_hash}`}
                                    target="_blank"
