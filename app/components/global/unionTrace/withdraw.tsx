...
import { useUnionTraceWithdraw } from "@/hooks/unionIndexer/withdraw";
...
const UnionTraceWithdraw = (props: TraceProps) => {
    const lst = props.lst ?? "babylon";
    const [showDetails, setShowDetails] = useState(true);
    const { data: unionTraceData } = useUnionTraceWithdraw(props.successTxHash);

    useEffect(() => {
        const getFinishHash = () => {
            let hash: string | undefined;
            if (unionTraceData?.delivery_success) {
                hash = unionTraceData?.delivery_traces?.find(v => v.type === "PACKET_ACK")?.transaction_hash ??
                    unionTraceData?.delivery_traces?.find(v => v.type === "PACKET_RECV")?.transaction_hash ??
                    unionTraceData?.delivery_traces?.find(v => v.type === "PACKET_SEND")?.transaction_hash;
            }
            return hash;
        }
        ...
        checkFinalHash();
    }, [props, unionTraceData?.delivery_success, unionTraceData?.delivery_traces]);

    return (
        <div className={`w-full flex flex-col ${props.className}`}>
            {unionTraceData?.withdraw_traces ?
                <Accordion
                    type="single"
                    collapsible
                        ...
                        <AccordionContent className="p-0">
                            <div className="flex items-center gap-4 pb-4">
                                ...
                                <div className="rounded-full aspect-square bg-white shadow-md p-2 shadow-escher-electricblue_light1">
                                    <Icon type="PiHandCoins" className="w-6 h-6 text-escher-electricblue" />
                                </div>
                                <div className="h-0.5 bg-escher-e4e8ed flex-1" />
                            ...
                            <div className={clsx(
                                ...
                                <TraceComp
                                    border="left"
                                    type="PACKET_SEND"
                                    traces={unionTraceData?.withdraw_traces}
                                    title={"Send packet"}
                                ...
                                <TraceComp
                                    border="right"
                                    type="PACKET_SEND"
                                    traces={unionTraceData?.delivery_traces}
                                    title={"Send packet"}
                                    isTransactionPage={props.isTransactionPage}
                                />
                                <TraceComp
...
export default UnionTraceWithdraw;