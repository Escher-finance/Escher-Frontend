...
const TokenSelection = (props: Props) => {
    ...
    return (
        <DialogEmpty open={open} onOpenChange={v => setOpen(v)}>
            <DialogTrigger disabled={!props.enabled} className="flex items-center border border-escher-dedfff dark:border-escher-darkblue_border bg-escher-f5f5ff dark:bg-escher-darkblue hover:bg-slate-50 rounded-full p-2 gap-4">
                ...
                {props.enabled &&
                    <IconCircle
                        icon="FaChevronDown"
                        className="bg-escher-electricblue_light4"
                        size="sm"
                    />
                }
            </DialogTrigger>
